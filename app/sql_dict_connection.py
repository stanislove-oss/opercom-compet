...
# ---------------------------------------------------------------------------
# ЗАМЕРЕНО в соседнем проекте на этой же базе (chizhik-compet,
# scripts/check_filters.py, август 2026): оба фильтра не режут ничего — под
# estat = 'R' и под cleaning_flag = 1 попадает 100% строк и 100% суммы.
# То есть база отдаётся уже очищенной, и на цифры отчёта фильтры сейчас
# не влияют.
#
# estat = 'R' оставлен включённым: он отбрасывает виртуальную рекламу —
# правило агентства «всегда ставить R». Сегодня это пустая операция, но если
# такие строки появятся, они не попадут в бюджеты молча. Колонка есть только
# в media_costs_union; в nat_tv, big_tv и reg_tv её нет, поэтому к рейтингам
# фильтр не применяется.
#
# cleaning_flag = 1 выключен: чистку здесь делает справочник, из него приходят
# include_exclude и competitor. Включённый фильтр срезал бы строки ещё до
# справочника, и понять, что пропало, было бы негде.
#
# Пустая строка = фильтр не ставится. Перепроверить: python scripts/check_filters.py
ESTAT_FILTER = _env("ESTAT_FILTER", "estat = 'R'")
